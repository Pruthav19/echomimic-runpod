...
def run_echomimic(image_path, audio_path, output_dir, user_params):
    ...
    cfg  = float(user_params.get("cfg_scale",        3.0))  # 3.0 balances lip sync + expression freedom (3.5 over-constrains)
    ...
    #   The EchoMimic source is already patched to animate the full face.
    #   0.15 gives generous room so cheek/jaw expressions aren't clipped.
    facemask_dilation = float(user_params.get("face_mask_dilation",   0.15))
...
def handler(event):
    try:
        ...
        #     Applied BEFORE face restoration so GFPGAN can clean up any warp edges.
        ...
        # 2c. Post-process: GFPGAN face restoration on every frame
        #     Fixes teeth/eye artefacts from the diffusion model + upscales 2× (512→1024).
        #     This replaces plain Real-ESRGAN — much better face quality.
        skip_enhance = input_data.get("skip_enhance", False)
        if not skip_enhance:
            from preprocess import restore_video_faces
            restored_path = os.path.join(job_dir, "output_restored.mp4")
            final_video_path = restore_video_faces(final_video_path, restored_path)
